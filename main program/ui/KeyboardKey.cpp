...
KeyboardKey::KeyboardKey(sf::Vector2f pos, sf::Vector2f size, std::string text, sf::Font & font) : keyText(text, font, 40), hitsText("0", font, 30)
{
	...
	this->keyText.setOrigin(this->keyText.getGlobalBounds().width / 2.f, 0.f);
	this->keyText.setPosition(this->background.getPosition().x, this->background.getPosition().y - this->background.getSize().y / 2);

	//this->hitsText.setString("274842");
	while (this->hitsText.getGlobalBounds().width >= this->background.getGlobalBounds().width - 15.f) {
		this->hitsText.setCharacterSize(this->hitsText.getCharacterSize() - 1);
	}
...
void KeyboardKey::SetStatKey(unsigned long long value)
{
	...
	if (this->hitsText.getGlobalBounds().width >= this->background.getGlobalBounds().width - 15.f) {
		this->hitsText.setCharacterSize(this->hitsText.getCharacterSize() - 1);
...
void KeyboardKey::SetHits(unsigned long long value)
{
	...
	this->hitsText.setOrigin(this->hitsText.getGlobalBounds().width / 2.f, this->hitsText.getGlobalBounds().height);
	this->hitsText.setPosition(this->background.getPosition().x, this->hitsText.getPosition().y);
...
void KeyboardKey::SetString(std::string str) {
	this->keyText.setString(str);
